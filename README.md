...
Personal study notebooks following **"Hands-On Machine Learning with Scikit-Learn and PyTorch" (2025)**, extended with custom CNN/RNN architectures from scratch, vision & multimodal transformers, g[...]
...
| 15 | Transformers for NLP & Chatbots — attention, positional encodings, multihead attention, transformer architecture, NMT, BERT (encoder-only), sentence embeddings, GPT-2 generation/QA, turni[...] |
| 16 | Vision & Multimodal Transformers — Vision Transformer (ViT) from scratch, fine-tuning `ViTForImageClassification` on Oxford-IIIT Pets, zero-shot classification & image-text similarity wit[...] |
| 17 | Large Language Models & Advanced LLM techniques — instruction tuning, parameter-efficient fine-tuning (e.g., LoRA), quantization & efficient inference, retrieval-augmented generation (RAG), and evaluation | `Chapter_17/Chapter_17.ipynb` |
| 18 | Autoencoders, GANs & Diffusion Models — stacked/tied/convolutional/sparse autoencoders, variational & discrete VAEs, GAN training loop, DDPM/DDIM diffusion from scratch, Stable Diffusion [...] |
...
| Mixed Precision & Quantization — FP16 training, `GradScaler`, dynamic/static/QAT quantization, 4-bit (NF4) quantized LLM inference with `bitsandbytes` | `Appendix/Mixed_Precision_Quantization.[...]` |
...
A minimal autograd engine (`micrograd/engine.py` + `micrograd/nn.py`) built from scratch — implements reverse-mode automatic differentiation over a scalar value graph, inspired by Andrej Karpat[...]

### Shared Utilities

`utils/utils.py` — reusable PyTorch helpers used across notebooks: CIFAR-10 data loaders with normalization, He-initialized MLP builder, and a generic train/evaluate loop with early stopping an[...]
